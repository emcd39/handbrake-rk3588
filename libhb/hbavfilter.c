    ...
    for (ii = 0; ii < hb_list_count(list); ii++)
    {
        ...
        if (settings != NULL)
        {
            ...
            if (hb_value_array_get(settings, 0) != NULL)
            {
                hb_dict_t * avfilter_settings_dict = hb_value_array_get(avfilter->settings, 0);
                hb_dict_t * cur_settings_dict = hb_value_array_get(settings, 0);
                if (cur_settings_dict && avfilter_settings_dict &&
                    hb_dict_get(avfilter_settings_dict, "vpp_rkrga") &&
                    hb_dict_get(cur_settings_dict, "vpp_rkrga"))
                {
            ...
            else if (hb_value_array_get(settings, 0) != NULL)
            {
